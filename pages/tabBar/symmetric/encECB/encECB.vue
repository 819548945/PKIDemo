<template>
	<view>
		<uni-forms ref="dynamicForm" :rules="dynamicRules" :modelValue="dynamicFormData">
			<uni-section title="step.1 填写待加密数据,密码,加密算法" type="line">
				<view class="example">
					...
					<uni-forms-item label="加密算法"  name="algorithm">
						...
						<view class="button-group">
							<button type="primary" size="max" @click="enc()">加密</button>	
						</view>
						<br>
			...
			enc:function() {
				...
				(ret1)=>{
					...
					(ret4)=>{
						...
						console.log(ret4.bytes)
						console.log(this.dynamicFormData.algorithm)
						this.st.encrypt(ret1.bytes,ret4.bytes,[],this.dynamicFormData.algorithm,
						(ret2)=>{
							console.log(ret2)
							if(ret2.code!=0){
							...
							(ret3)=>{
								...
								this.messageToggle('success','加密成功');
							})
						})
					})	
				})
