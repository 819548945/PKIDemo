<template>
	<view>
		<uni-forms ref="dynamicForm" :rules="dynamicRules" :modelValue="dynamicFormData">
			<uni-section title="step.1 填写加密数据,密码,加密算法" type="line">
				<view class="example">
					<uni-forms-item label="加密数据"  name="encData">
					...
					<uni-forms-item label="加密算法"  name="algorithm">
						...
						<br>
						<uni-forms-item label="解密后数据"  name="ori">
			...
			dec:function() {
				...
				(ret1)=>{
					...
					(ret4)=>{
						...
						this.st.decrypt(ret1.bytes,ret4.bytes,[],this.dynamicFormData.algorithm,
						(ret2)=>{
							console.log(ret2)
							if(ret2.code!=0){
