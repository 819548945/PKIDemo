<template>
	<view>
		<uni-forms ref="dynamicForm" :rules="dynamicRules" :modelValue="dynamicFormData">
			<uni-section title="step.1 填写待加密数据,密码,向量,加密算法" type="line">
				<view class="example">
				...
				(ret1)=>{
					...
					(ret4)=>{
						...
						(ret5)=>{
							...
							console.log("ori-"+ret1.bytes)
							console.log("pwd-"+ret4.bytes)
							console.log("iv-"+ret5.bytes)
							this.st.encrypt(ret1.bytes,ret4.bytes,ret5.bytes,this.dynamicFormData.algorithm,
							(ret2)=>{
								console.log(ret2)
								if(ret2.code!=0){
